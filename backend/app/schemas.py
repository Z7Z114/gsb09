...
def _require_non_blank(value: str) -> str:
    """必填字符串：去除首尾空白后必须非空，仅含空白视为非法。"""
    if not isinstance(value, str) or not value.strip():
        raise ValueError("必填字段不能为空或仅包含空白字符")
    return value.strip()
...
class CraftsmanBase(BaseModel):
    ...
    _check_name = field_validator("name", "school")(_require_non_blank)
...
class MessageBase(BaseModel):
    ...
    _check_content = field_validator("content")(_require_non_blank)
...
class WoodMaterialBase(BaseModel):
    ...
    _check_name = field_validator("name")(_require_non_blank)
